...
入口文件: `main.py` (单文件 Kivy 应用,无 .kv 模板,~720 行)。
...
`SAND_FILL_RATIO = 0.80`(顶部常量),应用到 3 处:`get_mound_top_y` / `redraw mound_top_base` / `_draw_top_sand sand_top_y`。起始上沙到上仓 80% 高、结束下堆到下仓 80% 高,各留 20% 余量——不留的话视觉太"满"显假,用户已明确要求。
